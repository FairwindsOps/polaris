...
func TestConfigWithCustomChecks(t *testing.T) {
	...
	isValid, err := parsedConf.CustomChecks["foo"].CheckObject(context.Background(), valid)
	assert.NoError(t, err)
	assert.Equal(t, true, isValid)
	isValid, err = parsedConf.CustomChecks["foo"].CheckObject(context.Background(), invalid)
	assert.NoError(t, err)

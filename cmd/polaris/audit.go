...
func init() {
	...
	auditCmd.PersistentFlags().BoolVar(&noColor, "no-color", false, "Whether to use color in pretty format.")
...
var auditCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		auditData := runAndReportAudit(cmd.Context(), config, auditPath, resourceToAudit, auditOutputFile, auditOutputURL, auditOutputFormat, noColor)
...
func runAndReportAudit(ctx context.Context, c conf.Configuration, auditPath, workload, outputFile, outputURL, outputFormat string, noColor bool) validator.AuditData {
	...
	if outputFormat == "score" {
		...
		outputBytes = []byte(auditData.GetPrettyOutput(!noColor))
	} else {
